                  ...
                  <tr v-for="(item, index) in filteredData" :key="item.idMenu">
                    ...
                    <td class="text-center">
                      ...
                      <button
                        ...
                        @click="deleteItem(item.idUomPackage)"
                      >
...
export default {
  ...
  setup() {
    ...
    function toggleForm(item) {
      if (item) {
        editIndex.value = item.idUomPackage;
        form.value.uomPackageName = item.uomPackageName;
      } else {
        editIndex.value = null;
        form.value.uomPackageName = "";
      }
    ...
    async function save() {
      try {
        ...
        if (editIndex.value !== null) {
          response = await instance.put(`/api/uompackages/${editIndex.value}`, {
            uomPackageName: form.value.uomPackageName,
          });
          const updatedItemIndex = data.value.findIndex(
            (item) => item.idUomPackage === editIndex.value
          );
          ...
          response = await instance.post("/api/uompackages", {
            uomPackageName: form.value.uomPackageName,
          });
    ...
    async function deleteItem(idUomPackage) {
      try {
        ...
        if (result.isConfirmed) {
          await instance.delete(`/api/uompackages/${idUomPackage}`);

          const index = data.value.findIndex(
            (item) => item.idUomPackage === idUomPackage
          );
          if (index !== -1) {
