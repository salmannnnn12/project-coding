    ...
    <div class="main-content">
      ...
      <div class="content-before-form" v-if="!showForm">
        <div class="intro-content">
          <div
            class="d-flex justify-content-between align-items-center pt-2 ms-2 position-relative"
          >
            <button
              v-if="hasPermissionForMenu('create')"
              type="button"
              ...
              <table class="table">
                ...
                <tbody>
                  <tr v-for="(item, index) in filteredData" :key="item.idMenu">
                    ...
                    <td>{{ item.menuName }}</td>
                    <td>{{ item.labelMenu }}</td>
                    <td class="text-center">
                      <button
                        v-if="hasPermissionForMenu('update', item.idMenu)"
                      ...
                      <button
                        v-if="hasPermissionForMenu('delete', item.idMenu)"
                        class="btn btn-danger btn-sm"
                        @click="deleteItem(item.idMenu)"
                      >
      ...
      <div class="form-wrapper" v-if="showForm">
        <div class="form-container">
          <form class="input" @submit.prevent="save">
            ...
            <div class="mb-3">
              <label for="labelMenu" class="form-label">Label</label>
              <input
                type="text"
                v-model="form.labelMenu"
                class="form-control"
            ...
            <button
              v-if="
                hasPermissionForMenu('create') || hasPermissionForMenu('update')
              "
              type="submit"
...
export default {
  ...
  setup() {
    ...
    async function fetchUserPermissions() {
      try {
        ...
        menuPermissionsArray.forEach((permission) => {
          ...
          permissions[permission.idMenu].createPermission =
            permission.createPermission;
          permissions[permission.idMenu].updatePermission =
            permission.updatePermission;
          permissions[permission.idMenu].deletePermission =
            permission.deletePermission;
          permissions[permission.idMenu].viewPermission =
    ...
    async function fetchData() {
      try {
        ...
        data.value = response.data.data.filter(
          (item) => hasPermissionForMenu("view", item.idMenu) || []
        );
    ...
    function hasPermissionForMenu(permissionType, idMenu = null) {
      if (idMenu === null) {
        return Object.values(menuPermissions.value).some(
          (permission) => permission[`${permissionType}Permission`] === 1
        );
      }
      return (
        (menuPermissions.value[idMenu] || {})[`${permissionType}Permission`] ===
        1
      );
    }
    ...
    function toggleForm(item) {
      if (item) {
        editIndex.value = item.idMenu;
        form.value.menuName = item.menuName;
        form.value.labelMenu = item.labelMenu;
      } else {
        editIndex.value = null;
        form.value.menuName = "";
        form.value.labelMenu = "";
      }
      showForm.value = !showForm.value;
    }
    ...
    async function save() {
      if (
        (editIndex.value !== null &&
          menuPermissions.value.updatePermission === 0) ||
        (editIndex.value === null &&
          menuPermissions.value.createPermission === 0)
      ) {
        return Swal.fire(
          "Unauthorized",
      ...
      try {
        ...
        if (editIndex.value !== null) {
          response = await instance.put(`/api/menus/${editIndex.value}`, {
            menuName: form.value.menuName,
            labelMenu: form.value.labelMenu,
          });
          const updatedItemIndex = data.value.findIndex(
            (item) => item.idMenu === editIndex.value
          );
          ...
          response = await instance.post("/api/menus", {
            menuName: form.value.menuName,
            labelMenu: form.value.labelMenu,
          });
    ...
    async function deleteItem(idMenu) {
      if (menuPermissions.value.deletePermission !== 1) {
        return Swal.fire(
          "Unauthorized",
          "You do not have permission to perform this action.",
          "error"
      ...
      try {
        ...
        if (result.isConfirmed) {
          await instance.delete(`/api/menus/${idMenu}`);
          const index = data.value.findIndex((item) => item.idMenu === idMenu);
          if (index !== -1) {
